 ...
 *
 * The `cn` utility (tailwind-merge) resolves class conflicts,
...
export class ZardDatePickerComponent implements ControlValueAccessor {
  ...
  protected readonly textClasses = computed(() => {
    const hasValue = !!this.value();
    return cn(!hasValue && 'text-muted-foreground');
  });

  protected readonly popoverClasses = computed(() => cn('w-auto p-0'));
